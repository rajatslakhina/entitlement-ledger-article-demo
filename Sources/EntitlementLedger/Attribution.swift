...
public struct LinkTap: Hashable, Sendable, Identifiable {
    ...
    /// Who tapped. Attribution is scoped per customer as well as per product:
    /// a tap by one person cannot earn a commission on someone else's purchase,
    /// and the seven-day clock runs independently for each of them.
    public let customerID: String

    public let productID: String
    public let occurredAt: Date

    public init(id: String, customerID: String, productID: String, occurredAt: Date) {
        self.id = id
        ...
        let qualifying = taps.values
            .filter {
                $0.productID == productID
                    ...
                    && window.contains(saleAt: saleDate, tappedAt: $0.occurredAt)
            }
            .sorted { lhs, rhs in
                lhs.occurredAt == rhs.occurredAt ? lhs.id < rhs.id : lhs.occurredAt < rhs.occurredAt
            }
