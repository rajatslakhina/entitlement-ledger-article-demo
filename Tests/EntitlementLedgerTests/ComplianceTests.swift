...
final class AttributionTests: XCTestCase {
    ...
    func testSaleJustInsideTheWindowIsCommissionable() {
        ...
        ledger.record(LinkTap(id: "tap-1", customerID: "mara", productID: "pro.annual", occurredAt: tapDate))
    ...
    func testSaleBeforeTheTapIsNotAttributed() {
        ...
        let outcome = ledger.attribute(saleOf: "pro.annual", by: "mara", at: tapDate.addingTimeInterval(-60))
        XCTAssertEqual(outcome, .unattributed)
    }
    ...
    func testMostRecentQualifyingTapWins() {
        var ledger = AttributionLedger()
        ledger.record([
            LinkTap(id: "tap-old", customerID: "mara", productID: "pro.annual", occurredAt: tapDate),
            LinkTap(id: "tap-new", customerID: "mara", productID: "pro.annual", occurredAt: tapDate.addingTimeInterval(3 * Clock.day))
        ])

        let outcome = ledger.attribute(saleOf: "pro.annual", by: "mara", at: tapDate.addingTimeInterval(4 * Clock.day))
    ...
    func testTapsAreScopedToTheirProduct() {
        var ledger = AttributionLedger()
        ledger.record([
            LinkTap(id: "tap-pro", customerID: "mara", productID: "pro.annual", occurredAt: tapDate),
            LinkTap(id: "tap-coach", customerID: "mara", productID: "coach.monthly",
                    occurredAt: tapDate.addingTimeInterval(9 * Clock.day))
        ])

        let outcome = ledger.attribute(saleOf: "pro.annual", by: "mara", at: tapDate.addingTimeInterval(10 * Clock.day))
        XCTAssertEqual(outcome, .unattributed)
    }

    /// A tap by one customer must not earn a commission on another customer's sale.
    func testTapsAreScopedToTheirCustomer() {
        var ledger = AttributionLedger()
        ledger.record(LinkTap(id: "tap-1", customerID: "mara", productID: "pro.annual", occurredAt: tapDate))

        let outcome = ledger.attribute(saleOf: "pro.annual", by: "jonas", at: tapDate.addingTimeInterval(Clock.day))
        XCTAssertEqual(outcome, .unattributed)
    }

    func testTapsAreIdempotent() {
        var ledger = AttributionLedger()
        let tap = LinkTap(id: "tap-1", customerID: "mara", productID: "pro.annual", occurredAt: tapDate)
